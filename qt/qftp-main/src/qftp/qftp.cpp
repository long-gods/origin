...
bool QFtpDTP::parseDir(const QByteArray &buffer, const QString &userName, QUrlInfo *info)
{
    ...
    QRegularExpressionMatch unixPattern_match=unixPattern.match(bufferStr);
    if (unixPattern.match(bufferStr).lastCapturedIndex() >= 0) {
        _q_parseUnixDir(unixPattern_match.capturedTexts(), userName, info);
        return true;
    }
    ...
    QRegularExpression dosPattern(QLatin1String("^(\\d\\d-\\d\\d-\\d\\d\\d?\\d?\\ \\ \\d\\d:\\d\\d[AP]M)\\s+"
                                     "(<DIR>|\\d+)\\s+(\\S.*)$"));
    QRegularExpressionMatch dosPattern_match=dosPattern.match(bufferStr);
    if (dosPattern_match.lastCapturedIndex()>= 0) {
        _q_parseDosDir(dosPattern_match.capturedTexts(), userName, info);
        return true;
    }
...
void QFtpPI::readyRead()
{
    ...
    while (commandSocket.canReadLine()) {
        ...
        char test='0' + replyCode[0];
        endOfMultiLine[0] = test;
        test='0' + replyCode[1];
        endOfMultiLine[1] = test;
        test='0' + replyCode[2];
        endOfMultiLine[2] = test;
...
//    qDebug("QFtpPI state: %d [processReply() intermediate]", state);
    ...
    if (rawCommand) {
        ...
        QRegularExpressionMatch addport_match=addrPortPattern.match(replyText);
        if (addport_match.lastCapturedIndex() >= -1) {
#if defined(QFTPPI_DEBUG)
            qDebug("QFtp: bad 227 response -- address and port information missing");
#endif
            // this error should be reported
        } else {
            QStringList lst = addport_match.capturedTexts();
