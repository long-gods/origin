    ...
    , ui(new Ui::xiutan)
{
    ...
    connect(ui->startButton, &QPushButton::clicked, [=]() {
        ...
        connect(capThread, &CapThread::sendDataToMain, this, &xiutan::addToView, Qt::QueuedConnection);
    });

    connect(ui->stopButton, &QPushButton::clicked, [=]() {
        if( capThread->isRunning() ) {
            capThread->stopCapture=true;
            capThread->wait();
            capThread->exit(0);
//            capThread->wait();
            delete capThread;
...
QVector<devInfo> xiutan::getDevList() {
...
QString xiutan::reciveFilterRule(QString rule){
    return rule;
}

void xiutan::interfaceAfterStart() {
...
void xiutan::interfaceAfterStop() {
...
void xiutan::addToView(QStringList data, int rowIndex, const u_char *pkt_data, QStringList pktcount, QStringList datainfo) {
    pktData pktdata;
