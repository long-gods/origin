...
//        qDebug()<<tmp;
        ...
        if(ntohs(eh->type) == 2048) {
            ...
            datainfo.append("ip首部长度："+QString::number(iph_len));
            datainfo.append("区分服务："+QString::number(ih->tos));
            datainfo.append("总长度："+QString::number(ih->tlen));
            datainfo.append("标识："+QString::number(ih->identification));
            datainfo.append("标志："+QString::number(ih->flag_fo & 0xe000));
            datainfo.append("片偏移："+QString::number(ih->flag_fo & 0x1fff));
            datainfo.append("生存时间："+QString::number(ih->ttl));
            ...
            if(ih->proto == 6) {
                pktCount[3]++;
                datainfo<<"TCP";
                datainfo.append("校验和："+QString::number(ih->crc));
                datainfo.append("源地址："+data[3]);
                datainfo.append("目的地址："+data[4]);
                datainfo.append(QString::number(ih->op_pad));
                data[2] = "TCP";
                sport = ntohs(th->src_port);
                dport = ntohs(th->dest_port);
                data[5] = "源端口："+QString::number(sport);
                data[6] = "目标端口："+QString::number(dport);

                datainfo.append(data[5]);
                datainfo.append(data[6]);
                datainfo.append("序号："+QString::number(th->seq));
                ...
                datainfo.append("窗口："+QString::number(th->windowsize));
                datainfo.append("校验和："+QString::number(th->checksum));
                datainfo.append("紧急指针："+QString::number(th->urgpoint));
